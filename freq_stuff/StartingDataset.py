...
class StartingDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, split, v_index = None, train_val_split=0.2):
        ...
        if split == "train":
            self.X = np.load(DATASET_PATH + "FREQ_X_train.npy") # (2115, 22, 1000)
            labels = np.load(DATASET_PATH + "FREQ_y_train.npy") - 769
            # self.train_indices = np.array(list(set(range(len(self.X))).difference(set(v_index))))
            ...
            #     for i in range(x_shape[0]):
                    ...
                    #     temp.append(sc.signal.cwt(self.X[i][j], sc.signal.ricker, widths))
                    # print("Done with instance ", i)
                    # newX.append(temp)
            #     self.X = np.array(newX)
            # np.save(DATASET_PATH + "FREQ_X_train.npy", self.X)
            # np.save(DATASET_PATH + "FREQ_y_train.npy", self.y)
            #self.X = np.reshape(self.X, (6768, 1, 264, 250))
            ...
            self.X = np.load(DATASET_PATH + "FREQ_X_valid.npy") # (2115, 22, 1000)
            labels = np.load(DATASET_PATH + "FREQ_y_valid.npy") - 769
            # self.val_indices = np.random.choice(self.X.shape[0], int(len(self.X) * train_val_split), replace=False)
            ...
            #         print("Done with instance ", i)
            #     self.X = np.array(newX)
            # np.save(DATASET_PATH + "FREQ_X_valid.npy", self.X)
            # np.save(DATASET_PATH + "FREQ_y_valid.npy", self.y)
            #self.X = np.reshape(self.X, (1692, 1, 264, 250))
            self.X = torch.from_numpy(self.X).double() 
            #self.X = transforms.Resize([224, 224])(self.X)
            self.y = labels.astype(np.int64) 
            self.person = np.load(DATASET_PATH + "person_train_valid.npy") # (2115, 1)
